...
export const runChainedCommands = (commands: string[]): string => {
  return execSync(commands.join(" && ")).toString().trim();
};
...
export const updateAllTestRepos = (): void => {
  ...
  Object.keys(exampleRepos).forEach((pkgManager) => {
...

    {
      ...
      if (repoUninstalledWithoutLockfileHasChanges) {
        console.log(`${repoPretty} has changes, will be updated...`);
        execSync(
...

    {
      ...
      if (repoUninstalledWithLockfileHasChanges) {
        execSync(
